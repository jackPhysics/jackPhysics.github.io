 ...
 function placePieceHere(event) //move the active piece into a new position
{
  ...
  if(mouseActive==1){
    ...
    if (piecesLeft[activeIndex] < 80 || piecesLeft[activeIndex] > 370 || piecesTop[activeIndex] < 80 || piecesTop[activeIndex] > 470) {//placed outside board
    ...
    ctx.beginPath();
    ctx.fillStyle = "Black";
    ctx.fillRect(offSet, gridSizeY * cellSize+offSet*1.1, gridSizeX * cellSize, offSet/4);
    ctx.closePath();
    ctx.beginPath();
    ...
    ctx.fillText("" + gamePc + "% completed",gridSizeX * cellSize+offSet/3, gridSizeY * cellSize+offSet*1.25);
    ctx.stroke();
    ctx.closePath();
