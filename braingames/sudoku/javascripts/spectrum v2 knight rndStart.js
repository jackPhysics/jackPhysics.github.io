...
function numberOfClicks(){
    let donePc = Math.floor(lineStage/64*100);
      ctx.beginPath();
      ctx.fillStyle = "Black";
      ctx.fillRect(4 * cellSize, 0 * cellSize + 10, cellSize*4, cellSize);
      ctx.closePath();
      ctx.beginPath();
      ctx.fillStyle = "White";
      ctx.font = "48px Arial";
      ...
      ctx.fillText(""+lineStage,3 * cellSize + 140, 0 * cellSize + 50);
      ctx.fillText(""+donePc+"%",5 * cellSize + 140, 0 * cellSize + 50);
      ctx.closePath();
}
