...
ctx.fillStyle = colorThis;
  ...
  // Set the fill style and draw a disc
  ctx.fillStyle = gradient;
  ctx.beginPath();
    ctx.arc(unitSize/2+circleEdge, unitSize/2+circleEdge, unitSize/2+circleEdge, 0, Math.PI*2);
    ctx.fill();
			  ...
			  ctx.beginPath();
				...
				//**right edge
				  ...
				  ctx.beginPath();
					...
					ctx.fill();
*/
if(unitsA[whatType*noOfItems+4]!="x"&&unitsA[whatType*noOfItems+4]!="y"){//not neutral
...
//countOutUnits=0;dumFont=24*unitScale;
  ...
  if (unitsA[whatType * noOfItems + 4] == "y") {
    //dummy
      ctx.beginPath();
      ...
      ctx.textAlign = "center";
      ctx.textBaseline = "middle";
      ctx.fillText("?",i2*unitSize+(60+40)*unitScale, j2*unitSize+(boxTop+boxHt/1.3)*unitScale);
      //ctx.stroke();
      ctx.closePath();
  }
  ...
  if (unitsA[whatType * noOfItems + 4] == "y") {
    //dummy
      ctx.beginPath();
      ctx.font = "bold 240px Arial";
    ctx.strokeStyle = "Red"; //textColorThis;
      ctx.textAlign = "center";
      ctx.textBaseline = "middle";
      ctx.lineWidth = "4";
      ctx.strokeText("?",i2*unitSize+(60+40)*unitScale, j2*unitSize+(boxTop+boxHt/1.3)*unitScale);
      //ctx.stroke();
      ctx.closePath();
  }
