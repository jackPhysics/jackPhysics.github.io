...
var rulesOfPlay = "orthogonal + diagonal";
var orthoFlag = true;
var diagFlag = true;
var colourOrder = "colours in order";
var maxColors = 2;
var startFlag=false;
...
function fillMenus(){
      ...
      var m2 = document.getElementById('ruleMenu');
      m2.innerHTML += '<a class="aiw" href="javascript:changeRepeat(0)">  orthogonal only </a>';
      m2.innerHTML += '<a class="aiw" href="javascript:changeRepeat(1)">  diagonal only </a>';
      m2.innerHTML += '<a class="aiw" href="javascript:changeRepeat(2)">  orthogonal + diagonal </a>';

      var m3 = document.getElementById('orderMenu');
      m3.innerHTML += '<a class="aiw" href="javascript:changeOrder(0)">  colours in order </a>';
      m3.innerHTML += '<a class="aiw" href="javascript:changeOrder(1)">  colours in order but remove empty colours </a>';
      m3.innerHTML += '<a class="aiw" href="javascript:changeOrder(2)">  random colours - equal chances</a>';
      m3.innerHTML += '<a class="aiw" href="javascript:changeOrder(3)">  random colours - weighted chances by amount</a>';
      m3.innerHTML += '<a class="aiw" href="javascript:changeOrder(4)">  random colours - inverse chances by amount</a>';
...
gameText.innerHTML = ''+maxColors+' colours, &nbsp;&nbsp;'+rulesOfPlay+', &nbsp; &nbsp;'+colourOrder+".&nbsp;&nbsp;";
colNo = maxColors;
}
...
function changeRepeat(m){
  ...
  orthoFlag = true;
  diagFlag = false;
  rulesOfPlay = "orthogonal only";}
  else if(m==1){
  orthoFlag = false;
  diagFlag = true;
  rulesOfPlay = "diagonal only";}
  else{
  orthoFlag = true;
  diagFlag = true;
  rulesOfPlay = "orthogonal + diagonal";}
    var gameText = document.getElementById('game2play');
    gameText.innerHTML = ''+maxColors+' colours, &nbsp;&nbsp;'+rulesOfPlay+', &nbsp; &nbsp;'+colourOrder+".&nbsp;&nbsp;";
}
...
function changeOrder(m){
  if(m==0){
  orderedColFlag =0;
  colourOrder = "colours in order";}
  else if(m==1){
  orderedColFlag =1;
  colourOrder = "colours in order but remove empty colours";}
  else if(m==2){
  orderedColFlag =2;
  colourOrder = " random colours - equal chances";}
  else if(m==3){
  orderedColFlag =3;
  colourOrder = "random colours - weighted chances by amount";}
  else{
  orderedColFlag =4;
  colourOrder = "random colours - inverse chances by amount";}
    var gameText = document.getElementById('game2play');
    gameText.innerHTML = ''+maxColors+' colours, &nbsp;&nbsp;'+rulesOfPlay+', &nbsp; &nbsp;'+colourOrder+".&nbsp;&nbsp;";
}
...
{
  if(startFlag&&!gameEndFlag){
    xpos = event.pageX;
    ypos = event.pageY;
    var xpos2 = xpos/boardSize*8/3;
    var ypos2 = ypos/boardSize*8/3;
    console.log("left:"+xpos+ " top:"+ypos+ " boardSize:"+boardSize+" left2:"+xpos2+ " top2:"+ypos2);

    var cellClickedX = Math.floor((xpos2-140)/80);
    var cellClickedY = Math.floor((ypos2-140)/80);
    var repeatFlag = false;
    //console.log("cellClickedX:"+cellClickedX+ " cellClickedY:"+cellClickedY);
    if(cellClickedX==oldCellX&&cellClickedY==oldCellY){
      repeatFlag = true;//clicking same square in consecutive turns
    }

if(cellClickedX>=0&&cellClickedY>=0&&cellClickedX<10&&cellClickedY<10){
if(!repeatFlag){
    if(board[cellClickedX][cellClickedY]!=indexColorCurrent){//no change if click same colour

    oldCellX = cellClickedX;
    oldCellY = cellClickedY;
    changeSingleCells(cellClickedX,cellClickedY);
    //removeSingleCells(cellClickedX,cellClickedY);
    drawGrid();
    drawCurrentCells();
...
/*
      ...
      ctx.lineWidth = "4";
    if(lineFlag){
      ctx.moveTo(lineStepsX[0],lineStepsY[0]);
      for(ld=0;ld<lineStage;ld++){
      ctx.lineTo(lineStepsX[ld],lineStepsY[ld]);}
      ctx.lineTo(xpos2, ypos2);
      ctx.stroke();
      ctx.closePath();
      lineStepsX[lineStage] = xpos2;
      lineStepsY[lineStage] = ypos2;
      lineStage++;}
    else{
      lineFlag=true;
      lineStepsX[0] = xpos2;
...
function placePicsX() //initialise, add the game pieces to the board
{
  ...
  for(i=0;i<unitTotal+1;i++){
    ...
    if(tileFlag){
      tileLabelsA[noOfImg] = letOfTile3[noOfTile];
    }
    else{
      tileLabelsA[noOfImg] = letOfTile3[noOfTile];//noOfTile;
    }
