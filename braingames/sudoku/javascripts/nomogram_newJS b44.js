 ...
 function placePieceHere(event) //move the active piece into a new position
{
    xpos = event.pageX;
    ypos = event.pageY;
    var xpos2 = xpos/boardSize*8/2;
    var ypos2 = ypos/boardSize*8/2;
    console.log("left:"+xpos+ " top:"+ypos+ " boardSize:"+boardSize+" left2:"+xpos2+ " top2:"+ypos2);
    //alert("noOfPieces:"+noOfPieces+ " unitTotal:"+unitTotal+ " lastIndex:"+lastIndex+ " gerNumber:"+gerNumber);
      /*
      ctx.beginPath();
      ctx.fillStyle = "Black"; //"White";
      //ctx.arc(xpos/boardSize-140/boardSize, ypos/boardSize-140/boardSize, 4, 0, Math.PI*2);
      ctx.arc(xpos2, ypos2, 4, 0, Math.PI*2);
      ctx.fill();
      ctx.closePath();
      */
          var cellClickedX = Math.floor((xpos2-320)/80);
          var cellClickedY = Math.floor((ypos2-320)/80);
          console.log("cellClickedX:"+cellClickedX+ " cellClickedY:"+cellClickedY);


              if(cellClickedX>=0&&cellClickedY>=0&&cellClickedX<20&&cellClickedY<20){

                changeSingleCells(cellClickedX,cellClickedY);
                drawGrid();

                gamePc = Math.round(gameDone / gameSize * 1000)/10;
                document.getElementById("percentage").innerHTML = "" + gamePc + "% completed";
            }
  ...
  if(mouseActive==1){
    ...
    var centreX1 = xpos;//  / boardSize;// - 40;
    var centreY1 = ypos;//  / boardSize;// - 40;
    console.log("centreX1=" + centreX1 + " centreY1=" + centreY1);
    var eighty = 80 * boardSize / 4;
    var centreX2 = Math.floor((centreX1-0)/eighty)*eighty+40*boardSize/4;
    var centreY2 = Math.floor((centreY1-0)/eighty)*eighty+40*boardSize/4;
    console.log("boardSize=" + boardSize);
    console.log("centreX2=" + centreX2 + " centreY2=" + centreY2);
    ...
    if(Math.abs(centreX1-centreX2)<(40*boardSize/4)){flagCentreX = true;
      centreX1 = (centreX2 - 10*boardSize/4);// * boardSize;
    }
    if(Math.abs(centreY1-centreY2)<(40*boardSize/4)){flagCentreY = true;
      centreY1 = (centreY2 - 10*boardSize/4);// * boardSize;
    }
    console.log("centreX1=" + centreX1 + " centreY1=" + centreY1);
    ...
    piecesLeft[activeIndex]= Math.round((centreX1-((pieceSize)/2-2)*boardSize))/boardSize;//*boardSize
    piecesTop[activeIndex]= Math.round((centreY1-((pieceSize)/2-2)*boardSize))/boardSize;//*boardSize
    console.log("piecesLeft[activeIndex]=" + piecesLeft[activeIndex] + " piecesTop[activeIndex]=" + piecesTop[activeIndex]);
    }
    else{
    ...
    if (piecesLeft[activeIndex] < 80 || piecesLeft[activeIndex] > 470 || piecesTop[activeIndex] < 80 || piecesTop[activeIndex] > 470) {//placed outside board
      if (firedA[activeIndex] == 1) {//was inside board
        firedA[activeIndex] = 0;
        gameDone--;
        gamePc = Math.round(gameDone / gameSize * 100);
        document.getElementById("percentage").innerHTML = "" + gamePc + "% completed";
       }
    }
    else {//placed inside board
      if (firedA[activeIndex] == 0) {//firedA holds if piece outside (=0) or inside board (=1)
        firedA[activeIndex] = 1;
        gameDone++;
        gamePc = Math.round(gameDone / gameSize * 100);
        document.getElementById("percentage").innerHTML = "" + gamePc + "% completed";
     }
    ...
    ctx.beginPath();
    ctx.fillStyle = "Black";
    ctx.fillRect(offSet, gridSizeX * cellSize+offSet*1.1, gridSizeX * cellSize, offSet/4);
    ctx.closePath();
    ctx.beginPath();
    ...
    ctx.fillText("" + gamePc + "% completed",gridSizeY * cellSize-offSet/2, gridSizeX * cellSize+offSet*1.25);
    ctx.stroke();
    ctx.closePath();
...
function placePics() //removing pieces
{
...
function placePicsX() //initialise, add the game pieces to the board
{
...
var thisNumb = 0;
  ...
  for(k=0;k<noOfImages;k++){
    ...
    if(Math.random()<0.5){
  imageArray[k].style.left=Math.floor(k/noPerPile)*pieceSize*2+shiftX+randShift1+"px";
  piecesLeft[k] = Math.round(Math.floor(k/noPerPile)*pieceSize*2+shiftX+randShift1);
  }
    else{
  imageArray[k].style.left=Math.floor(k/noPerPile)*pieceSize*2+shiftX-randShift1+"px";
  piecesLeft[k] = Math.round(Math.floor(k/noPerPile)*pieceSize*2+shiftX-randShift1);}
    if(Math.random()<0.5){
  imageArray[k].style.top=(k-Math.floor(k/noPerPile)*noPerPile)*pieceSize*0+shiftY+randShift2+"px";
  piecesTop[k] = Math.round((k-Math.floor(k/noPerPile)*noPerPile)*pieceSize*0+shiftY+randShift2);
  }
    else{
  imageArray[k].style.top=(k-Math.floor(k/noPerPile)*noPerPile)*pieceSize*0+shiftY-randShift2+"px";
  piecesTop[k] = Math.round((k-Math.floor(k/noPerPile)*noPerPile)*pieceSize*0+shiftY-randShift2);}
  ...
  nextNumb++;
  if(thisNumb==noPerPile){thisNumb=0;nextNumb++}
  }
...
function changeSize() //change the size of board and pieces following key press
{
  ...
  var scale2 = boardSize;
	var swidth2=swidth*scale2/4;
	var	sheight2=sheight*scale2/4;
