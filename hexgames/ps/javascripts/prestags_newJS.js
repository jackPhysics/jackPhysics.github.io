 ...
 function placePieceHere(event) //move the active piece into a new position
{
  ...
  if(mouseActive==1){
    ...
    keyBoard.style.left= xpos - relX/2 + hopSignL*pieceSize*boardSize*4 + "px";//xpos - relX/2 + "px";
    keyBoard.style.top= ypos - relY/2 + hopSignT*pieceSize*boardSize*4 + "px";//ypos - relY/2 + "px";
    }
    else{
    ...
    keyBoard.style.left= xpos - relX + hopSignL*pieceSize*boardSize*4 + "px";//xpos - relX/2 + "px";
    keyBoard.style.top= ypos - relY + hopSignT*pieceSize*boardSize*4 + "px";//ypos - relY/2 + "px";
    }
    hopperLeft= (xpos - relX + hopSignL*pieceSize*boardSize*4);//xpos - relX/2 + "px";
    hopperTop= (ypos - relY) + hopSignT*pieceSize*boardSize*4;//ypos - relY/2 + "px";
...
function placePics() //initialise, add the game pieces to the board
{
  ...
  for(i=0;i<unitTotal+1;i++){
    ...
    pieceNameA[noOfImg] = unitsA[i*noOfItems+1].slice(0,2);
    //alert(""+pieceNameA[i]);
    imageArray[noOfImg].setAttribute("id", "piece#"+noOfImg);//1st piece is id:#0
    imageArray[noOfImg].setAttribute("src", unitLocation+unitsA[i*noOfItems]);//+unitExtension);//".jpg");//where all units the same
    //imageArray[noOfImg].setAttribute("src", "units/"+unitsA[i*noOfItems]+".jpg");//where all units the same
    //if units are different then need "units/"+unitsA[i*noOfItems]+(j+1)+".JPG"
	  imageArray[noOfImg].setAttribute("title", "#"+(noOfImg+1)+": "+unitsA[i*noOfItems+1]+""+unitsA[i*noOfItems+2]+" ("+(j+1)+"/"+totalNo+")");
	...
	//if(armourA[noOfImg]=="s"){firerA[noOfImg]=1; navyMisA[noOfImg]=0;}
	//if(armourA[noOfImg]=="a"){isCavA[noOfImg]=2; airMisA[noOfImg]=0;}
	if(armourA[noOfImg]=="c"){isCavA[noOfImg]=1;}
	if(armourA[noOfImg]=="f"){firerA[noOfImg]=1;}
	if(armourA[noOfImg]!="f"){firerA[noOfImg]=0;}
	if(armourA[noOfImg]=="l"){leaderA[noOfImg]=1;
    var dummyL = unitsA[i*noOfItems+1].slice(0,1);
    leaderValueA[noOfImg]=dummyL*1;
  }
...
function virtualKey(k){
	...
	if(keyText==3){
    if(chooseD6==0){rollBinaryTable();}
    else if(chooseD6==1){rollDieTable();}
    else if(chooseD6==2){rollOddsTable();}
    else{}
    //rollOddsTable();
    //rollDieTable();
...
function saveWindow(){
  ...
  var thisShift=0;
    ...
    if(dismountA[k]==1){
      swCav = findName(imageArray[activeIndex], activeIndex);
...
function boardUp()
{
  var loadBox = document.getElementById("sizebox").value;
  loadBox = 1*loadBox;
  try {  var multiThis2 = multiThisNow;  }//unique size of each board
  catch(err) {  var multiThis2 = 1;  }
  /*if(multiThisNow != 'undefined'||multiThisNow != null){
  var multiThis = 1;}
  else{
  var multiThis = multiThisNow;}*/
  var multiThis = 1;
  if(isNaN(loadBox)){}
  else{multiThis=loadBox;}
  if(multiThis>5){multiThis=5;}
  //console.log(""+loadBox+" "+multiThis);
  //boardMult=boardMult*multiThis;
  boardMult=multiThis*multiThis2;
  //document.getElementById("btnBS").innerHTML = "Size x"+boardMult;
  loadBox.value = ""+multiThis;
  setupBoard();
}
