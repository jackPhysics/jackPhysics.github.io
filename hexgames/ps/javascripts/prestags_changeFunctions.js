...
function changeDice(){
	if(chooseD6==0){
		chooseD6=1;
		document.getElementById("btn612").innerText = "now: d6";
		document.getElementById("diceResult").innerText = "d6";
	}
		else if(chooseD6==1){
			chooseD6=2;
			document.getElementById("btn612").innerText = "now: odds";
			document.getElementById("diceResult").innerText = "odds";
		}
		else{
				chooseD6=0;
				document.getElementById("btn612").innerText = "now: d2";
				document.getElementById("diceResult").innerText = "binary";
			}
}
...
if(dieInHand==1){
  ...
  document.getElementById("diceResult").textContent = "" + odds+ "or more";//"You rolled " + noPips;
	//document.getElementById("rollBtn").setAttribute("value", "pick up dice");
	dieInHand=0;
	}
else{
	document.getElementById("diceResult").textContent = "Odds?";
...
//if(keyText=="x"||keyText=="X"){ //unit destroyed
        ...
        piecesTop[pieceIndex]= deadPosTop/boardSize;
			...
			if(pieceIndex<gerNumber){
				gerVP=Number(gerVP)+Number(unitsA[thisIsA[pieceIndex]*itemsPerUnit+4]);}
			else{alliedVP=Number(alliedVP)+Number(unitsA[thisIsA[pieceIndex]*itemsPerUnit+4]);}
			}
...
for(s=gerNumber;s<neutNumber;s++){
	...
	document.getElementById('btn24').innerText="supply B - all";
	}
	else if(supplyB==1){
for(s=gerNumber;s<neutNumber;s++){
		if(panicA[s]==0){
		...
		document.getElementById('btn24').innerText="supply B - none";
	}
	else if(supplyB==2){
for(s=gerNumber;s<neutNumber;s++){
		if(Math.random()<0.5){
		...
		document.getElementById('btn24').innerText="supply B - random";
	}
	//**THIS ROUTINE FLIPS SUPPLY TO OPPOSITE
/*for(s=0;s<gerNumber;s++){
			changeSupply(imageArray[s], s, 1);	//becuse call it in loop need 'return' in changeSupply() to come back here
...
function victoryPoints(){
  victoryLevel();
  alert(nameA+" VP: "+unionVP+" ("+Bpanic+")\n"+nameB+" VP: "+rebelVP+" ("+Apanic+")\n"+vicLevel+" to "+victor);
...
for(i=0;i<unitTotal+1;i++){
	var totalNo=unitsA[i*noOfItems+2];
	for(j=0;j<totalNo;j++){
...
function reloadSetupSizeDown(){
	setupMult = setupMult*0.9;
	if(setUpGameNo==0){setUpGameNo=maxNoOfSetUps-1;}
	else{setUpGameNo--;}
	if(setUpGameNo<0){setUpGameNo=0;}
	reloadSetup();
}

function reloadSetupSizeUp(){
	setupMult = setupMult*1.1;
	if(setUpGameNo==0){setUpGameNo=maxNoOfSetUps-1;}
	else{setUpGameNo--;}
	if(setUpGameNo<0){setUpGameNo=0;}
	console.log(setUpGameNo);
	reloadSetup();
}
		...
		function reload2(d){
			...
			reScale=size/loadSize*setupMult;
			var flipIndex=0;
		  ...
		  var noPieces=noOfImages;
		    ...
		    dismountDummyA[activeIndex]=loadA[m+8];
				if(dismountDummyA[activeIndex]!="x"&&dismountDummyA[activeIndex]!=" x"&&dismountDummyA[activeIndex]!=" x "&&dismountDummyA[activeIndex]!=0){
					dismountA[activeIndex]=1;
		        //alert(""+dismountDummyA[activeIndex]+" "+activeIndex+" "+k);
						//alert("x "+dismountDummyA[activeIndex]+" "+activeIndex+" "+dismountA[activeIndex]);
		      }
		      else{
		        dismountA[activeIndex]=0;
						//alert("no "+dismountDummyA[activeIndex]+" "+activeIndex+" "+dismountA[activeIndex]);
		      }
