...
//if(keyText=="x"||keyText=="X"){ //unit destroyed
        ...
        piecesTop[pieceIndex]= deadPosTop/boardSize;
			...
			if(pieceIndex<gerNumber){
				unionVP=Number(unionVP)+Number(unitsA[thisIsA[pieceIndex]*itemsPerUnit+2]);}
			else{rebelVP=Number(rebelVP)+Number(unitsA[thisIsA[pieceIndex]*itemsPerUnit+2]);}
			}
  	...
  	if(hideA[activeIndex]!=1){
		if(deadA[activeIndex]==0){
			...
			if(activeIndex<gerNumber){
				...
				deadA[activeIndex]=1; //1=dead non-armour
				console.log("dead > gerNumber");
				console.log("deadA[activeIndex]="+deadA[activeIndex]);
				activeImage.style.width=(pieceSize*boardSize)/2+"px";
				activeImage.style.height=(pieceSize*boardSize)/2+"px";
				activeImage.style.left=(piecesLeft[activeIndex]*boardSize)+(pieceSize*boardSize)/4+"px";
				activeImage.style.top=(piecesTop[activeIndex]*boardSize)+(pieceSize*boardSize)/4+"px";
    ...
    if(exitA[activeIndex]==1){exitA[activeIndex]=0;}
		...
		if(panicA[activeIndex]!=1){
			...
			if(activeIndex<gerNumber){//assign points for exiting
				...
				else{var vp=unitsA[thisIsA[activeIndex]*itemsPerUnit+2];
...
activeImage = imageArray[activeIndex];
	...
	if(hideA[activeIndex]!=1){//if not hidden
		...
		activeImage.style.borderColor="cyan";}//artillery
		if(disruptedA[activeIndex]==3){
		activeImage.style.borderColor="silver";}//fired
		...
		activeImage.style.borderColor="white";}
		if(exitA[activeIndex]==1){
		activeImage.style.borderColor="green";}//white";
...
function victoryPoints(){
  victoryLevel();
  if(startingPlayer==nameA){
    alert(nameA + " VP: " + rebelVP + " \n" + nameB + " VP: " + unionVP + " \n" + vicLevel +  victor);
  }
  else{
    alert(nameA+" VP: "+unionVP+" \n"+nameB+" VP: "+rebelVP+" \n"+vicLevel+victor);}
...
if(startingPlayer==nameA){
  ...
  if(vL>=3){
  vicLevel = "Decisive Victory to ";}
  else if(vL>=2){
  vicLevel = "Substantive Victory to ";
  }
  else if(vL>=1){
  vicLevel = "Marginal Victory to ";}
  else{
  vicLevel = "";}
    ...
    keyBoard.style.top= thisY + hopSignT*pieceSize*Math.pow(boardSize,0.5)*6 + "px";//ypos - relY/2 + "px";
		...
		function saveWindow2(){
			...
			var thisShift=0;
			 ...
			 allText=allText+unionVP+","+rebelVP+","+turnNumb+","+ boardSize +","+ turnLetter +","+ turnNumber +","+situationAddress;
		...
		if(armourA[imgIndex]=="n"||armourA[imgIndex]=="c"||armourA[imgIndex]=="w"||armourA[imgIndex]=="a"){
			hideA[imgIndex]=1; //flag unit as hidden
			activeImage.setAttribute("title", "possible US unit...");
			//activeImage.setAttribute("src", "units/neut/m_blank1.jpg");
			firerA[imgIndex]=0;
			printUnitBlank(imgIndex, typeNumbA[imgIndex], typeTotalA[imgIndex], thisIsA[imgIndex]);
			}
			else {
			hideA[imgIndex]=1; //flag unit as hidden
			activeImage.setAttribute("title", "possible US unit...");
		...
		if(armourA[imgIndex]=="n"){//||armourA[imgIndex]=="d"){//hide inf
			hideA[imgIndex]=1; //flag unit as hidden
			//push data onto stack Array
			var dummyAddress = 	activeImage.getAttribute("src");
			var dummyTitle = activeImage.getAttribute("title");
			//alert(""+dummyAddress+" "+dummyTitle+" "+hidJapUnitAlen);
			hidJapUnitA[hidJapUnitAlen*3]=dummyAddress;
			hidJapUnitA[hidJapUnitAlen*3+1]=dummyTitle;//push onto stack
			hidJapUnitA[hidJapUnitAlen*3+2]=imgIndex;//index number of piece
			hidJapUnitAlen++;//stack length ++
			//alert(""+hidJapUnitAlen);
			activeImage.setAttribute("title", "possible Japanese unit...");
			//activeImage.setAttribute("src", "units/neut/j_blank1.jpg");
			printUnitBlank(imgIndex, typeNumbA[imgIndex], typeTotalA[imgIndex], thisIsA[imgIndex]);
			}
			else if(armourA[imgIndex]=="a"){//hide art
			hideA[imgIndex]=1; //flag unit as hidden
			var dummyAddress = 	activeImage.getAttribute("src");
			var dummyTitle = activeImage.getAttribute("title");
			//alert(""+dummyAddress+" "+dummyTitle);
			//push onto stack
			hidJapArtA[hidJapArtAlen*3]=dummyAddress;//src address of piece
			hidJapArtA[hidJapArtAlen*3+1]=dummyTitle;//text titles of piece
			hidJapArtA[hidJapArtAlen*3+2]=imgIndex;//index number of piece
			hidJapArtAlen++;//stack length ++
			activeImage.setAttribute("title", "possible Japanese unit...");
			//activeImage.setAttribute("src", "units/neut/j_blank1.jpg");
			printUnitBlank(imgIndex, typeNumbA[imgIndex], typeTotalA[imgIndex], thisIsA[imgIndex]);
			}
			else {//hide dummy - dont push onto stack
			hideA[imgIndex]=1; //flag unit as hidden
			activeImage.setAttribute("title", "possible Japanese unit...");
			//activeImage.setAttribute("src", "units/neut/j_blank1.jpg");
			printUnitBlank(imgIndex, typeNumbA[imgIndex], typeTotalA[imgIndex], thisIsA[imgIndex]);
	 ...
	 activeImage.setAttribute("title", "#"+(imgIndex+1)+": "+unitsA[thisIsA[imgIndex]*noOfItems+1]+" "+unitsA[thisIsA[imgIndex]*noOfItems+6]+"-"+unitsA[thisIsA[imgIndex]*noOfItems+7]+" "+typeNumbA[imgIndex]+"/"+unitsA[thisIsA[imgIndex]*noOfItems+2]+ unitNamesA[imgIndex]);
		printUnit(imgIndex, typeNumbA[imgIndex], typeTotalA[imgIndex], thisIsA[imgIndex]);hideA[imgIndex]=0;
		if(armourA[imgIndex]=="a"){firerA[imgIndex]=1;}
		doEdgeColor();
		//activeImage.styleborderColor="black";
		}
			else{//unhide jap unit
				if(unhideInf){//unhide jap inf unit
					var dummyFrac = 0.3;//(japInfTListA[turnNumber]+3)/(japInfTListA[turnNumber]*2+3);
					//alert(""+dummyFrac);
					if(Math.random()<dummyFrac||hidJapUnitAlen==0){//dummy unit or no real units on the stack
					activeImage.setAttribute("src", "units/jap/dummy1.jpg");
					activeImage.setAttribute("title", "#"+(imgIndex+1)+": "+"Japan dummy unit"+" "+"\n...?");
					hideA[imgIndex]=0;
					openA[imgIndex]=true;
					//activeImage.style.borderColor="black";
					}
					else{//real unit
					//var randUnit = Math.floor(Math.random()*japInfTListA[turnNumber]);
					var randUnit = Math.floor(Math.random()*hidJapUnitAlen);//-1
					//alert(""+hidJapUnitAlen+" "+randUnit);
					//activeImage.setAttribute("src", "units/"+japInfA[randUnit*3]+".jpg");
					//activeImage.setAttribute("title", "#"+(imgIndex+1)+": "+japInfA[randUnit*3+1]+" "+japInfA[randUnit*3+2];)
					activeImage.setAttribute("src", hidJapUnitA[randUnit*3]);
					activeImage.setAttribute("title", hidJapUnitA[randUnit*3+1]);
					var indexHere = hidJapUnitA[randUnit*3+2];
					hidJapUnitA[randUnit*3]=hidJapUnitA[(hidJapUnitAlen-1)*3];//load the unhidden place with the last place
					hidJapUnitA[randUnit*3+1]=hidJapUnitA[(hidJapUnitAlen-1)*3+1];
					hidJapUnitA[randUnit*3+2]=hidJapUnitA[(hidJapUnitAlen-1)*3+2];
					hidJapUnitAlen--;
					hideA[imgIndex]=indexHere;//0; hideA>1 shows it has been hidden and unhidden -not its index but index of piece it has become
					//alert(""+indexHere);
					openA[imgIndex]=true;
					if(armourA[indexHere]=="a"){firerA[imgIndex]=2;}
					doEdgeColor();
					//activeImage.style.borderColor="black";
					}

				}
				else{//unhide jap art unit
					var dummyFrac = 0.3;//(japInfTListA[turnNumber]+3)/(japInfTListA[turnNumber]*2+3);
					//alert(""+dummyFrac);
					if(Math.random()<dummyFrac||hidJapArtAlen==0){//dummy unit or no real units on the stack
					activeImage.setAttribute("src", "units/jap/dummy1.jpg");
					activeImage.setAttribute("title", "#"+(imgIndex+1)+": "+"Japan dummy unit"+" "+"\n...?");
					hideA[imgIndex]=imgIndex;//0;
					openA[imgIndex]=true;
					activeImage.style.borderColor="black";
					}
					else{//real unit
					//var randUnit = Math.floor(Math.random()*japInfTListA[turnNumber]);
					var randUnit = Math.floor(Math.random()*hidJapArtAlen);//-1
					//activeImage.setAttribute("src", "units/"+japInfA[randUnit*3]+".jpg");
					//activeImage.setAttribute("title", "#"+(imgIndex+1)+": "+japInfA[randUnit*3+1]+" "+japInfA[randUnit*3+2];)
					activeImage.setAttribute("src", hidJapArtA[randUnit*3]);
					activeImage.setAttribute("title", hidJapArtA[randUnit*3+1]);
					var indexHere = hidJapArtA[randUnit*3+2];
					hidJapArtA[randUnit*3]=hidJapArtA[(hidJapArtAlen-1)*3];
					hidJapArtA[randUnit*3+1]=hidJapArtA[(hidJapArtAlen-1)*3+1];
					hidJapArtA[randUnit*3+2]=hidJapArtA[(hidJapArtAlen-1)*3+2];
					hidJapArtAlen--;
					hideA[imgIndex]=indexHere;
					openA[imgIndex]=true;
					if(armourA[indexHere]=="a"){firerA[imgIndex]=2;}
					doEdgeColor();
					//activeImage.style.borderColor="black";
		...
		if(transA[imgIndex]>0){
				//if(oneOffHide==1){
				//  document.all[transporterA[imgIndex]].setAttribute("src", "units/"+unitsA[thisIsA[transporterA[imgIndex]]*noOfItems]);
				//  document.all[transporterA[imgIndex]].setAttribute("title", unitsA[thisIsA[transporterA[imgIndex]]*noOfItems+1]+" "+typeNumbA[transporterA[imgIndex]]+"/"+unitsA[thisIsA[transporterA[imgIndex]]*noOfItems+3]+" "+unitsA[thisIsA[transporterA[imgIndex]]*noOfItems+2]);
				//  hideA[transporterA[imgIndex]]=0; //this and previous 2 lines unhides the passenger
				//}
				titles = activeImage.getAttribute("title");
				ps=transporterA[imgIndex];
				activeImage.setAttribute("title", "LOADED UP: " + titles);
				title1 = activeImage.getAttribute("title");
				title2 = imageArray[ps].getAttribute("title");
				activeImage.setAttribute("title", title1+" carrying: "+ title2);
				}
				if(transA[imgIndex]==1){
				activeImage.style.borderColor="blue";}
				else if(transA[imgIndex]==2){
				activeImage.style.borderColor="white";}
		...
		hideRus=1;
		hideForces();
		document.getElementById("btn23").innerText="Show U.S.";}
		else{
		hideRus=0;
		unHideForces();
		document.getElementById("btn23").innerText="Hide U.S.";}
		...
		hideGer=1;
		hideForces();
		document.getElementById("btn24").innerText="Show Jap";
		}
		else{
		hideGer=0;
		unHideForces();
		document.getElementById("btn24").innerText="Hide Jap";
		}
		}
		...
		hideForces();
		unHideForces();
		document.getElementById("btn25").innerText="Hide U.S./Show Jap";
		}
		else{
		hideGer=0;
		hideRus=1;
		...
		hideForces();
		unHideForces();
		document.getElementById("btn25").innerText="Show U.S./Hide Jap";
		}
		}
