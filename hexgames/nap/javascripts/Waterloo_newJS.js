...
var size=8; //biggest size counts as size 8
...
var turnPos = 0; //this is where in seasonA array we are
 ...
 function placePieceHere(event) //move the active piece into a new position
{
  ...
  if(mouseActive==1){
    ...
    //alert("left:"+xpos+ " top:"+ypos);
    if(mouseOverFlag){
    activeImage.style.left= xpos - relX/2  + "px";//xpos - relX*relN  + "px";//xpos - relX/2 + "px";
    activeImage.style.top= ypos - relY/2  + "px";//ypos - relY*relN  + "px";//ypos - relY/2 + "px";
...
function placePics() //initialise, add the game pieces to the board
{
  ...
  for(i=0;i<unitTotal+1;i++){
    ...
    if (cmbtFactorsA[noOfImg].slice(0, 1) == "(") {
      cmbtFactorsMaxA[noOfImg] = cmbtFactorsA[noOfImg].slice(1, 2);//assumes munber is between 1 and 9
    }
    else{
    ...
    printUnit(noOfImg, j+1, totalNo, i);

    if(unitsA[i*noOfItems+5]=="hq"){
    	  imageArray[noOfImg].setAttribute("title", "#"+(noOfImg+1)+": "+unitsA[i*noOfItems+1]+", "+designateLA[noOfImg]+", "+designateRA[noOfImg]+": "+hqA[noOfImg]+" Corps, "+unitsA[i*noOfItems+6]+"-"+unitsA[i*noOfItems+7]+" ("+(j+1)+"/"+totalNo+") ");
      }
      else{
	  imageArray[noOfImg].setAttribute("title", "#"+(noOfImg+1)+": "+unitsA[i*noOfItems+1]+", "+designateLA[noOfImg]+" "+designateRA[noOfImg]+", "+unitsA[i*noOfItems+6]+"-"+unitsA[i*noOfItems+7]+" ("+(j+1)+"/"+totalNo+") ");
