...
function printUnit(n, b, t, id){//(n, b, t, id, c0)//n=number of image b=battalion number, t=Total, id=number of unit,
...
/*
  if(noOfImgPrnt>=neutNumber-1){
   colorThis = colorN;
   textColorThis = textColorN;
   classFactor = classFactor-neutNumber;
   boxColor=textColorN;
   colorThisBT = colorN;
  }
  else if(noOfImgPrnt>=gerNumber){
   colorThis = coloring[4];
   var textColorThis = coloring[5];
   classFactor = classFactor-gerNumber;
   boxColor=coloring[6];
   colorThisBT = coloring[7];
  }
*/
...
var classType = classFactor;//unit number - out of all
    ...
    //TOP RIGHT - REPEATED AGAIN AT BOTTOM SO GOES ON TOP OF UNIT-TYPE BOX
    countOutUnits=0;
    ctx.beginPath();
    dumFont=55*unitScale;
    ctx.font = "bold "+dumFont+"px Arial";
    //ctx.font = "30px Arial";
    if(unitsA[whatType*noOfItems+4]=="a"){
    ctx.fillStyle = "black";//textColorThis;
    ctx.textAlign = "right";
    ctx.textBaseline = "top";
    ctx.lineWidth = "8";
    ctx.fillRect(i2*unitSize+(unitSize/2+edgeSize*1)*unitScale,j2*unitSize+boxTop/2+edgeSize*0.5*unitScale, 28, 34);//25   35
    //ctx.fillRect(i2*unitSize+(unitSize-edgeSize*3.3)*unitScale,j2*unitSize+edgeSize*unitScale, 30, 50);//25   35
    //ctx.fillText(""+rangeA[noOfImgPrnt],i2*unitSize+(unitSize-edgeSize*1.5)*unitScale,j2*unitSize+edgeSize*1.5*unitScale);//25   35
    ctx.closePath();
    ctx.beginPath();
    ...
    ctx.textBaseline = "top";
    ctx.lineWidth = "4";
    ctx.fillText("2",i2*unitSize+(unitSize/2+edgeSize*1.2)*unitScale,j2*unitSize+boxTop/2+edgeSize*0.5*unitScale);//25   35
    //ctx.fillText("2",i2*unitSize+(unitSize-edgeSize*1.5)*unitScale,j2*unitSize+edgeSize*1.5*unitScale);//25   35
    ctx.closePath();}
        ...
        ctx.textBaseline = "bottom";
        ctx.lineWidth = "2";
        ctx.fillText(""+"",i2*unitSize+edgeSize*1.5*unitScale,j2*unitSize+(unitSize-edgeSize*1.5)*unitScale);//25   35
    ...
    //if(countOutUnits>loopNoXtra){break;}
        //ctx.stroke();
            ...
            &&unitsA[whatType*noOfItems+5]!="sw"&&unitsA[whatType*noOfItems+5]!="spt"
              && unitsA[whatType * noOfItems + 5] != "dra" && unitsA[whatType * noOfItems + 5] != "hq2"
              && unitsA[whatType * noOfItems + 5] != "scout" && unitsA[whatType * noOfItems + 5] != "huss"
              && unitsA[whatType * noOfItems + 5] != "war"&& unitsA[whatType * noOfItems + 5] != "rock") {
                ctx.beginPath();
                ctx.lineWidth = "4";
                ctx.strokeStyle = colorThisBT;
            ...
            ||unitsA[whatType*noOfItems+5]=="sw"){
              ctx.beginPath();
              ctx.lineWidth = "4";
            ...
            //acav and cav
            if(unitsA[whatType*noOfItems+5]=="acav"||unitsA[whatType*noOfItems+5]=="cav"||unitsA[whatType*noOfItems+5]=="hart"){
                ctx.beginPath();
                ctx.lineWidth = "4";
                ctx.fillStyle = colorThisBT;
                ctx.moveTo(i2*unitSize+(60+80)*unitScale, j2*unitSize+boxTop*unitScale);
            ...
            //dragoons, mounted infantry
                if(unitsA[whatType*noOfItems+5]=="dra"){
                      ctx.beginPath();
                      ctx.lineWidth = "4";
                      ctx.strokeStyle = colorThisBT;
                      ctx.moveTo(i2*unitSize+(60+40)*unitScale, j2*unitSize+(boxTop+boxHt/2)*unitScale);
                      ctx.lineTo(i2*unitSize+(60+80)*unitScale, j2*unitSize+(boxTop+boxHt)*unitScale);
                      ctx.stroke();
                      ctx.closePath();
                      ctx.beginPath();
                ...
                if(unitsA[whatType*noOfItems+5]=="spear"){
                 ...
                 //light cav
                if(unitsA[whatType*noOfItems+5]=="hb"||unitsA[whatType*noOfItems+5]=="scout"||unitsA[whatType*noOfItems+5]=="recon"){
                    ctx.beginPath();
                    ctx.lineWidth = "4";
                    ctx.fillStyle = colorThisBT;
                    ctx.moveTo(i2*unitSize+(60+80)*unitScale, j2*unitSize+boxTop*unitScale);
                    ...
                    ctx.closePath();}
                  //hussar
               if(unitsA[whatType*noOfItems+5]=="huss"){
                   ctx.beginPath();
                   ctx.lineWidth = "4";
                   ctx.fillStyle = colorThisBT;
                   ctx.moveTo(i2*unitSize+(60+80)*unitScale, j2*unitSize+boxTop*unitScale);
               ...
               //warrior
            if(unitsA[whatType*noOfItems+5]=="war"){
                ctx.beginPath();
                ctx.lineWidth = "4";
                ctx.fillStyle = colorThisBT;
                ctx.moveTo(i2*unitSize+(60+80)*unitScale, j2*unitSize+boxTop*unitScale);
                ...
                ctx.closePath();
                ctx.beginPath();
                ctx.moveTo(i2*unitSize+(60+30)*unitScale, j2*unitSize+(boxTop+boxHt/4)*unitScale);
                ctx.lineTo(i2*unitSize+(60+60)*unitScale, j2*unitSize+(boxTop+boxHt/4*3)*unitScale);
                ctx.stroke();
                ctx.closePath();
                    ctx.beginPath();
                    ctx.moveTo(i2*unitSize+(60+35)*unitScale, j2*unitSize+(boxTop+boxHt/5)*unitScale);//+10
                    ctx.lineTo(i2*unitSize+(60+15)*unitScale, j2*unitSize+(boxTop+boxHt/4)*unitScale);//+18
                    ctx.lineTo(i2*unitSize+(60+25)*unitScale, j2*unitSize+(boxTop+boxHt/2.3)*unitScale);//+30
                    ctx.lineTo(i2*unitSize+(60+37)*unitScale, j2*unitSize+(boxTop+boxHt/3.3)*unitScale);//+30
                    ctx.lineTo(i2*unitSize+(60+35)*unitScale, j2*unitSize+(boxTop+boxHt/5)*unitScale);//+10
                    ctx.stroke();
                    ctx.closePath();
            }
  ...
  //arm, mech, arm cav, spa
            if(unitsA[whatType*noOfItems+5]=="arm"||unitsA[whatType*noOfItems+5]=="mech"||unitsA[whatType*noOfItems+5]=="acav"||unitsA[whatType*noOfItems+5]=="spa"){
                ctx.beginPath();
                ctx.lineWidth = "4";
                ctx.strokeStyle = colorThisBT;
            ...
            //guard
                 if(unitsA[whatType*noOfItems+5]=="guard"){
                     ...
                     ctx.closePath();
                       ...
                       ctx.textAlign = "center";
                       ctx.textBaseline = "middle";
                       ctx.fillText("G",i2*unitSize+(60+40), j2*unitSize+(boxTop+30), 40);
                       ctx.stroke();
                       ctx.closePath();
                 }
            else if(unitsA[whatType*noOfItems+5]=="mar"){
                ctx.beginPath();
                ctx.lineWidth = "4";
                ctx.strokeStyle = colorThisBT;
            ...
            if(unitsA[whatType*noOfItems+5]=="hq"){
              ...
              ctx.fillStyle = colorThisBT;//colorThis;
              ctx.textAlign = "center";
              ctx.textBaseline = "middle";
              ctx.fillText(""+hqA[noOfImgPrnt],i2*unitSize+(60+40)*unitScale, j2*unitSize+(boxTop+boxHt/1.875)*unitScale);
              //ctx.fillText("HQ",i2*unitSize+(60+40)*unitScale, j2*unitSize+(boxTop+boxHt/1.875)*unitScale);
              ctx.stroke();
              ctx.closePath();
            }
            if(unitsA[whatType*noOfItems+5]=="hq2"){
              ctx.beginPath();
              ctx.lineWidth = "3";
              ctx.strokeStyle = colorThisBT;
              ctx.arc(i2*unitSize+(60+40)*unitScale, j2*unitSize+(boxTop+boxHt/2)*unitScale, 35*unitScale, 0, 2*Math.PI);
            ...
            if(unitsA[whatType*noOfItems+5]=="lvt"){
             ...
             //support troops
            if(unitsA[whatType*noOfItems+5]=="spt"){
              ctx.beginPath();
              ctx.lineWidth = "3";
              ctx.strokeStyle = colorThisBT;
...
//countOutUnits=0;dumFont=24*unitScale;
    ...
    ctx.fillStyle = textColorThis;
        ctx.textAlign = "center";
        ctx.textBaseline = "middle";
       if(unitsA[whatType*noOfItems+5]=="hq"||unitsA[whatType*noOfItems+5]=="hq2"){
        ctx.fillText(""+unitsA[whatType*noOfItems+7],i2*unitSize+100*unitScale,j2*unitSize+155*unitScale);
      }
      else{
       ctx.fillText(""+cmbtFactorsA[noOfImgPrnt]+"-"+unitsA[whatType*noOfItems+7],i2*unitSize+100*unitScale,j2*unitSize+155*unitScale);
        ...
        //if(countOutUnits>loopNoXtra){break;}
            ...
            ctx.textAlign = "center";
            ctx.textBaseline = "middle";
            ctx.fillText(""+designateRA[noOfImgPrnt],i2*unitSize+220*unitScale,j2*unitSize+unitSize/5*3.9*unitScale, 100);
            ctx.translate(150, 150);
            ctx.rotate(Math.PI/2);
            ctx.translate(-150, -150);
            ...
            ctx.closePath();}
                        ...
                        ctx.closePath();
                          ctx.beginPath();
                              //unit size symbol
                              ctx.font = "bold 36px Arial";
                              ctx.fillStyle = textColorThis;//textColorBig;
                                  ctx.textAlign = "center";
                                  ctx.textBaseline = "middle";
                                  //ctx.fillText(""+unitsA[whatType*noOfItems+9],i2*unitSize+100,j2*unitSize+42);
                                  ctx.fillText(""+unitsA[whatType*noOfItems+1],i2*unitSize+100*unitScale,j2*unitSize+boxTop*2/3*unitScale,140);
                                  ctx.closePath();
                      }
else{//if neutral
  //**top text
  ...
  ctx.textBaseline = "middle";
  ctx.lineWidth = "2";
  ctx.fillText(""+unitsA[whatType*noOfItems+5],i2*unitSize+100*unitScale,j2*unitSize+85*unitScale, 160);
  ...
  //bottom text
      ...
      ctx.fillStyle = textColorThis;
          ctx.textAlign = "center";
          ctx.textBaseline = "middle";
          ctx.fillText(""+unitsA[whatType*noOfItems+6],i2*unitSize+100*unitScale,j2*unitSize+155*unitScale,160);
...
//countOutUnits=0;
    ...
    ctx.fillStyle = textColorThis;
        ...
        ctx.fillText(""+unitsA[whatType*noOfItems+8],i2*unitSize+100*unitScale,j2*unitSize+boxTop*3/4*unitScale);
        }
        //countOutUnits++;
        //if(countOutUnits>loopNoXtra){break;}
