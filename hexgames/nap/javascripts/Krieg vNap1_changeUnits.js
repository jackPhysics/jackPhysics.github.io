function dropPoints(id){
  var noToDrop = id;
  if (cmbtFactorsA[noToDrop].slice(0, 1) == "(") {
    cmbtFactorsA[noToDrop] = cmbtFactorsA[noToDrop].slice(1, 2);//assumes munber is between 1 and 9
    cmbtFactorsA[noToDrop]--;
    if(cmbtFactorsA[noToDrop]<0){cmbtFactorsA[noToDrop]=0;}
    ...
    printUnit(noToDrop, typeNumbA[noToDrop], typeTotalA[noToDrop], thisIsA[noToDrop]);
  }
  else{
  if(cmbtFactorsA[noToDrop]!="X"){
  cmbtFactorsA[noToDrop]--;
  if(cmbtFactorsA[noToDrop]<1){cmbtFactorsA[noToDrop]="X";}
  imageArray[noToDrop].setAttribute("title", "#"+(noToDrop+1)+": "+cmbtFactorsA[noToDrop]+"-"+unitsA[thisIsA[noToDrop]*noOfItems+7]+" "+unitsA[thisIsA[noToDrop]*noOfItems+3]+"pts ("+(typeNumbA[noToDrop])+"/"+typeTotalA[noToDrop]+")");
  document.getElementById('hopText').innerText=""+activeImage.getAttribute("title");
  printUnit(noToDrop, typeNumbA[noToDrop], typeTotalA[noToDrop], thisIsA[noToDrop]);
}}
