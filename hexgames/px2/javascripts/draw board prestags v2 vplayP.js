...
ctx.fillStyle = setupsA[nh+4];
var dumX = setupsA[nh+1];
var dumY = setupsA[nh+2];
var dumR = setupsA[nh+3];
var xPos = (dumX+brdSftCoX)*(hexD*3)+brdEdge;//Math.floor(dumX*(hexD*3)+brdEdge);//+hexD/2;
if(dumY%2==1){xPos=xPos+(hexD/2+hexD);}
var yPos = (dumY+brdSftCoY)*(hexLong)+brdEdge;//Math.floor(dumY*(hexLong)+brdEdge);//+hexLong;
var posNowX = xPos+hexD/2;
var posNowY = yPos+hexLong;
var posNowR = dumR*2*hexLong;
  //alert(""+setupsA[nh+1]+" "+setupsA[nh+2]);
ctx.globalAlpha = 0.4;
  ctx.beginPath();
  ctx.arc(posNowX, posNowY, posNowR, 0, Math.PI*2);
  ctx.fill();
  ctx.closePath();
ctx.globalAlpha = 1;
}
...
ctx.font = "48px Arial";
ctx.fillStyle = locationsA[nh+3];
ctx.textAlign = "center";
ctx.textBaseline = "middle";
ctx.lineWidth = "3";
var dumX = locationsA[nh+1];
var dumY = locationsA[nh+2];
var xPos = (dumX+brdSftCoX)*(hexD*3)+brdEdge;//Math.floor(dumX*(hexD*3)+brdEdge);//+hexD/2;
if(dumY%2==1){xPos=xPos+(hexD/2+hexD);}
var yPos = (dumY+brdSftCoY)*(hexLong)+brdEdge;//Math.floor(dumY*(hexLong)+brdEdge);//+hexLong;
var posNowX = xPos+hexD/2;
...
ctx.restore();}
  ...
  //alert(""+locationsA[nh+1]+" "+locationsA[nh+2]);
  ctx.beginPath();
  ctx.moveTo(posNowX-hexD/10,posNowY+hexD/2);
  ctx.lineTo(posNowX+hexD/10*4,posNowY-hexD/2);
  ctx.lineTo(posNowX-hexD/10,posNowY-hexD/2);
  ctx.lineTo(posNowX-hexD/10*2,posNowY);
  ctx.lineTo(posNowX+hexD/10*2,posNowY);
  //ctx.lineTo(posNowX-hexD/10*2,posNowY-hexD/5);
  //ctx.lineTo(posNowX+hexD/10*2,posNowY-hexD/5);
  ctx.fill();
  ctx.closePath();
  ctx.beginPath();
  ...
  ctx.lineTo(posNowX-hexD/10,posNowY-hexD/2);
  ctx.lineTo(posNowX-hexD/10*2,posNowY);
  ctx.lineTo(posNowX+hexD/10,posNowY);
  //ctx.lineTo(posNowX-hexD/10*2,posNowY-hexD/5);
  //ctx.lineTo(posNowX+hexD/10*2,posNowY-hexD/5);
  ctx.stroke();
  ctx.closePath();
  ctx.beginPath();
  ctx.moveTo(posNowX,posNowY-hexD/2+hexD/10);
  ctx.lineTo(posNowX-hexD/10*4,posNowY-hexD/2+hexD/10);
  ctx.lineTo(posNowX-hexD/10*6,posNowY+hexD/10);
  ctx.lineTo(posNowX-hexD/10*2,posNowY+hexD/10);
  //ctx.lineTo(posNowX-hexD/10*4,posNowY-hexD/2+hexD/10*4);
  //ctx.lineTo(posNowX-hexD/10,posNowY-hexD/2+hexD/10*4);
  ctx.lineTo(posNowX,posNowY-hexD/2+hexD/10);
  ctx.fill();
  ctx.closePath();
  ctx.beginPath();
  ctx.moveTo(posNowX,posNowY-hexD/2+hexD/10);
  ...
  ctx.beginPath();
  ctx.moveTo(posNowX-hexD/10,posNowY+hexD/2);
  ctx.lineTo(posNowX+hexD/10*4.5,posNowY-hexD/2-hexD/10);
  ctx.stroke();
  ctx.closePath();
  //ctx.fillRect(posNowX,posNowY,hexD,hexD);
  //ctx.closePath();
}
}
