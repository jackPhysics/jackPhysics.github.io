...
var startYear=2001;
var changeUnitA=new Array();//holds array to show if unit has changed
...
var flipBrdNoBY=0;
var mirrBrdNoR=1;
var mirrBrdNoB=1;//boards MUST be mirrored
var stepFlag = false;
var redArmyNo = 0;
var blackArmyNo = 0;
var gameNo = 0;
...
function findNewBoard2(bad)//board address
{
      ...
      var mirrLetB = "L";
        ...
        flipBrdNoBY=0;
/*
  if(blackBno == bad) { //new board number same as old board number
        flipBrdNoBX=flipBrdNoBX+5;
        if(flipBrdNoBX>20){flipBrdNoBY=flipBrdNoBY+5;flipBrdNoBX=0;}
        //flipBrdNoBY=flipBrdNoBY+5;
        if(flipBrdNoBY>25){flipBrdNoBY=0;}
      }
  else{
    flipBrdNoBX=0;
    flipBrdNoBY=0;
...
function placePics() //initialise, add the game pieces to the board
{
  ...
  //alert(""+hopperHTMLcode);
      ...
      var armyNow = allTheArmiesA[b];
      m1.innerHTML += '<a class="aiw" href="javascript:changeBlackArmy2('+b+')">Army #'+b+': '+armyNow+'</a>';
      }

      var m2 = document.getElementById('armyMenu2');
      for(a=0;a<redArmyMax;a=a+2){
      //document.getElementById('armyMenu').insertAdjacentHTML='<a class="save" href="javascript:changeRedArmy2("'+a+'")">"Army #"'+a+'</a>';
...
/*
      ...
      for(d=1;d<boardMax+1;d=d+2){
     //document.getElementById('armyMenu').insertAdjacentHTML='<a class="save" href="javascript:changeRedArmy2("'+a+'")">"Army #"'+a+'</a>';
      var boardNow = allTheBoardsA[d];
      m4.innerHTML += '<a class="aiw" href="javascript:findNewBoard2('+d+')">Board #'+d+': '+boardNow+'</a>';
      }
*/
      //board.style.width = swidth + "px";
      //board.style.height = sheight + "px";
...
*/
  ...
  for(i=0;i<unitTotal+1;i++){
    ...
    //console.log(""+i+" "+j);
    countOutUnits0++;
    changeUnitA[noOfImg]=0;//means this is the original unit
    ...
    if(unitsA[i*noOfItems+5]!="supply"||unitsA[i*noOfItems+4]!="x"){
      hasSteps[noOfImg]=true;
        ...
        var dummyAFvalue = unitsA[i*noOfItems+6].slice(0,1);//get 1st character
        var dummyAFvalue2 = unitsA[i*noOfItems+6].slice(1);//get all but 1st character
        //console.log(""+dummyAFvalue2);
        if(dummyAFvalue=="?"){//untried unit
    ...
    if(armourA[noOfImg]=="f"){firerA[noOfImg]=1;}
  ...
  imageArray[k].style.top=Math.floor(k/10)*pieceSize*1.5+200+"px";//k*60+"px";
  ...
  imageArray[k].style.left=(k-Math.floor(k/10)*10)*pieceSize*1.5+200+"px";//(k-(Math.floor(k/10)*10))+100+"px";//k*60+"px";
  imageArray[k].style.top=Math.floor(k/10)*pieceSize*1.5+600+"px";//k*60+"px";
  piecesLeft[k] = Math.round((k-Math.floor(k/10)*10)*pieceSize*1.5+200);
  ...
  imageArray[k].style.top=Math.floor(k-neutNumber)*pieceSize*1.5+350+"px";//k*60+"px";
  piecesLeft[k] = pieceSize*1.5+20;
...
function placePics2() //print new armies
{
  ...
  for(k=0;k<noOfImages;k++){
    if(k<gerNumber){
      reScale=size/8;
      piecesLeft[k]=1*designateLA[2*k]*reScale*1;//.42;
      piecesTop[k]=1*designateLA[2*k+1]*reScale*1;//.4;
/*
  imageArray[k].style.left=(k-Math.floor(k/10)*10)*pieceSize*1.5+200+"px";//(k-(Math.floor(k/10)*10))+100+"px";//k*60+"px";
  imageArray[k].style.top=Math.floor(k/10)*pieceSize*1.5+200+"px";//k*60+"px";
  piecesLeft[k] = Math.round((k-Math.floor(k/10)*10)*pieceSize*1.5+200);
  ...
  */
  ...
  imageArray[k].style.zIndex = nextZ-1;}
      ...
      reScale=size/8;
      piecesLeft[k]=1*designateRA[2*k]*reScale*1;//.42;
      piecesTop[k]=1*designateRA[2*k+1]*reScale*1;
  /*
  imageArray[k].style.left=(k-Math.floor(k/10)*10)*pieceSize*1.5+200+"px";//(k-(Math.floor(k/10)*10))+100+"px";//k*60+"px";
  imageArray[k].style.top=Math.floor(k/10)*pieceSize*1.5+600+"px";//k*60+"px";
  piecesLeft[k] = Math.round((k-Math.floor(k/10)*10)*pieceSize*1.5+200);
  ...
  */
  ...
  imageArray[k].style.zIndex = nextZ-1;}
      ...
      reScale=size/8;
      var kn2 = k-neutNumber;
      piecesLeft[k]=1*designateLN[2*kn2]*reScale*1;//.42;
      piecesTop[k]=1*designateLN[2*kn2+1]*reScale*1;//.4;
      /*
  imageArray[k].style.left=pieceSize*1.5+20+"px";//(k-(Math.floor(k/10)*10))+100+"px";//k*60+"px";
  imageArray[k].style.top=Math.floor(k-neutNumber)*pieceSize*1.5+350+"px";//k*60+"px";
  piecesLeft[k] = pieceSize*1.5+20;
...
{
  var targetImg = n;
  var dumIndex = targetImg.getAttribute("id");
  var localActiveIndex = dumIndex.slice(6);

if(mouseOverFlag){//n is a HTMLCanvasElement, not an index
  //console.log("n="+n+" targetImg="+targetImg+" localActiveIndex="+localActiveIndex)
...
if(keyText=="c"||keyText=="C"){
  ...
  else if(untriedFlag[activeIndex]){
    untriedTry(activeIndex);}
/*  else if(unitsA[thisIsA[activeIndex]*noOfItems+10]=="MI"||unitsA[thisIsA[activeIndex]*noOfItems+10]=="LB"||unitsA[thisIsA[activeIndex]*noOfItems+10]=="MC"
          ||unitsA[thisIsA[activeIndex]*noOfItems+10]=="HC"||unitsA[thisIsA[activeIndex]*noOfItems+10]=="US"){
    changeUnitA[activeIndex]++;
    if(changeUnitA[activeIndex]>1){changeUnitA[activeIndex]=0;}
...
doEdgeColor();
  ...
  if(keyText==87){ //switch on/off piece expand on mouseOver
      if(mouseOverFlag){mouseOverFlag=false;}
      else{mouseOverFlag=true;}
      }
...
function saveWindow(){
  ...
  var thisShift=0;
    ...
    if(dismountA[k]==1){
      if(leaderA[activeIndex]>0){
        swCav = leaderA[activeIndex];
      }
      else{swCav = changeUnitA[activeIndex];}
      //cmbtFactorsA[activeIndex]
      ...
      if(leaderA[activeIndex]>0){
        ...
        else{swCav = changeUnitA[activeIndex];}//cmbtFactorsA[activeIndex]
      //swCav = "x";
      }
    ...
    allText=allText+Apanic+","+Bpanic+",";

  	//llText=allText+redArmyNo+","+blackArmyNo+","+redBno+","+ flipBrdNoR +","+ blackBno +","+ flipBrdNoB+",";
    //allText=allText+hideHills+","+hideWoods+","+hideTowns+","+ hideRivers +","+ hideRoads +","+ hideLakes+","+hideSwamps+",";
    allText=allText+hideHills+","+hideWoods2+","+hideTowns+","+ hideRivers +","+ hideRoads +","+ hideLakes2+","+hideSwamps2+",";
    //hideLetters  hexCoordsFlag  centreDotFlag  don't need recording

    allText=allText+canvW0+","+canvW+","+canvH0+","+ canvH +",";

    allText=allText+redArmyNo+","+blackArmyNo+","+flipBrdNoBX+","+ flipBrdNoBY +","+ blackBno +","+ flipBrdNoB+",";
...
if(loadA[loopTill-14-shiftBackwards]=="true"){
  hideRivers = true;
}
if(loadA[loopTill-15-shiftBackwards]=="true"){
  hideTowns = true;
}
...
Bpanic = 1*loadA[loopTill-18-shiftBackwards];
Apanic = 1*loadA[loopTill-19-shiftBackwards];
...
/*
  ...
  hideRivers = loadA[loopTill-14-shiftBackwards];
  hideTowns = loadA[loopTill-15-shiftBackwards];
  hideWoods = Boolean(loadA[loopTill-16-shiftBackwards]);
...
*/
  ...
  initBoard();
  /*
  var shiftNumbOld = shiftNumb;
  shiftNumb = flipBrdNoBX;
    moveBoard(1);
  shiftNumb = flipBrdNoBY;
    moveBoard(3);
  shiftNumb = shiftNumbOld;
*/
/*
  if(flipBrdNoBX==-5){
    moveBoard(2);
  }
  else if(flipBrdNoBX>0){
    var shiftLoops = Math.round(flipBrdNoBX/5);
    for(sl=0;sl<shiftLoops;sl++){
      moveBoard(1);
    }
  }
  if(flipBrdNoBY==-5){
  ...
  else if(flipBrdNoBY>0){
    var shiftLoops = Math.round(flipBrdNoBY/5);
    for(sl=0;sl<shiftLoops;sl++){
      moveBoard(3);
    }
  }
...
document.getElementById("turnBox").innerText = currentPlayer+"\nTurn "+dummyTurnNo+" (of "+maxTurns+")";//+"-"+Number(yearNow+1);
document.getElementById("timebox").innerText = currentPlayer+"\nTurn "+dummyTurnNo+" (of "+maxTurns+")";//+"-"+Number(yearNow+1);
if(turnTock==2){alert("Turn "+dummyTurnNo+" (of "+maxTurns+")"+" for "+currentPlayer+"\nscore: A="+unionVP+" vp vs B="+rebelVP+" vp");
turnTock=0;
saveWindow2();}
//statusNow();
}
