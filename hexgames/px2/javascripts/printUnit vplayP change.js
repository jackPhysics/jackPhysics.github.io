...
//0=colours;1=class letter;2=points;3=number of units;4=n,f,l,c;5=box symbol;
//6=CF;7=MF;8=fire attack;9=fire range;10=type code
var printData = [unitsA[whatType*noOfItems+0],unitsA[whatType*noOfItems+1],unitsA[whatType*noOfItems+2],unitsA[whatType*noOfItems+3],
unitsA[whatType*noOfItems+4],unitsA[whatType*noOfItems+5],unitsA[whatType*noOfItems+6],unitsA[whatType*noOfItems+7],
unitsA[whatType*noOfItems+8],unitsA[whatType*noOfItems+9],unitsA[whatType*noOfItems+10]];


var activeImageDum = document.getElementById("piece#"+noOfImgPrnt);
imageArray[noOfImgPrnt].setAttribute("title", "#"+(noOfImgPrnt+1)+": "+unitsA[whatType*noOfItems+10]+" "+unitsA[whatType*noOfItems+6]+"-"+unitsA[whatType*noOfItems+7]+" fire:"+unitsA[whatType*noOfItems+8]+"/"+unitsA[whatType*noOfItems+9]+" (pts:"+unitsA[whatType*noOfItems+2]+")("+batNumb+"/"+batTot+")");
document.getElementById('hopText').innerText=""+activeImageDum.getAttribute("title");
if(changeUnitA[noOfImgPrnt]>0){
  printData = changeUnitType(activeImageDum, noOfImgPrnt, printData);
}
//console.log(""+printData);

  if(leaderA[noOfImgPrnt]>0){
      switch (leaderA[noOfImgPrnt]){
      case 1:
      leaderBonus = "(9)";
      leaderRange = "(2)";
      break;
  ...
  imageArray[noOfImgPrnt].setAttribute("title", "#"+(noOfImgPrnt+1)+": "+unitsA[whatType*noOfItems+1]+" "+unitsA[whatType*noOfItems+6]+" "+unitsA[whatType*noOfItems+10]+" "+leaderBonus+"-"+leaderRange+" (pts:"+unitsA[whatType*noOfItems+2]+") ("+batNumb+"/"+batTot+")");
  }
...
var classType = classFactor;//unit number - out of all
    ...
    //ctx.fillText(""+unitsA[whatType*noOfItems+9],i2*unitSize+(unitSize-edgeSize*1.5)*unitScale,j2*unitSize+edgeSize*1*unitScale);}//25   35
      ...
      //ctx.fillText(""+leaderRange,i2*unitSize+(unitSize-edgeSize*1.5)*unitScale,j2*unitSize+edgeSize*1*unitScale);
    }
...
*/
  ...
  ctx.fillRect(i2*unitSize+60*unitScale, j2*unitSize+boxTop*unitScale, 80*unitScale, boxHt*unitScale);
    ...
    //if(countOutUnits>loopNoXtra){break;}
        //ctx.stroke();
var dummyHoldType = unitsA[whatType*noOfItems+5];
unitsA[whatType*noOfItems+5] = printData[5];
            //**unit symbol
            //no inf symbol
            ...
            if(unitsA[whatType*noOfItems+5]=="eng"){
             ...
             //engineers
            if(unitsA[whatType*noOfItems+5]=="bi"){
              ctx.beginPath();
              ctx.lineWidth = "3";
              ctx.font = "bold 64px Arial";
              ctx.fillStyle = "Black";//colorThisBT;
              ctx.textAlign = "center";
              ctx.textBaseline = "middle";
            hexstring = "\u{1f5e1}";
              ctx.fillText(hexstring,i2*unitSize+(60+40)*unitScale, j2*unitSize+(boxTop+boxHt/1.875)*unitScale);
...
unitsA[whatType*noOfItems+5] = dummyHoldType;
//class letter (combat factor for depot, air)
//countOutUnits=0;dumFont=24*unitScale;
    ...
    ctx.fillStyle = textColorThis;
        ...
        //else{//if depot or air
        //ctx.fillText(""+cmbtFactorsA[noOfImgPrnt],i2*unitSize+100*unitScale,j2*unitSize+155*unitScale);
        //}
        if(printData[5]!="air"&&printData[5]!="depot"){
        if(leaderA[noOfImgPrnt]>0){//no class-letter if leader
        ctx.fillText(""+leaderRange,i2*unitSize+100*unitScale,j2*unitSize+155*unitScale);
            }
        else{
        ctx.fillText(""+printData[1],i2*unitSize+100*unitScale,j2*unitSize+155*unitScale);}
        }
        else{//if depot or air
        ctx.fillText(""+cmbtFactorsA[noOfImgPrnt],i2*unitSize+100*unitScale,j2*unitSize+155*unitScale);
        }
...
//countOutUnits=0;
    dumFont=36*unitScale;
    if(changeUnitA[noOfImgPrnt]==1){
      dumFont=30*unitScale;
    }
    else{dumFont=36*unitScale;}
    ctx.font = "bold "+dumFont+"px Arial";
    //ctx.font = "36px Arial";
    //ctx.fontWeight = "bold";
    ctx.fillStyle = textColorThis;
        ...
        ctx.fillText(""+printData[10],i2*unitSize+100*unitScale,j2*unitSize+boxTop*2/3*unitScale);//+unitsA[whatType*noOfItems+8]
