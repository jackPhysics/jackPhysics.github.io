...
function changeDice(){
	if(chooseD6){
		chooseD6=false;
		document.getElementById("btn612").innerText = "choose d2";
		document.getElementById("diceResult").innerText = "choose d2";
	}
		else{
			chooseD6=true;
...
//if(keyText=="x"||keyText=="X"){ //unit destroyed
    ...
    imageArray[pieceIndex].setAttribute("title", "#"+(pieceIndex+1)+": "+unitsA[thisIsA[pieceIndex]*noOfItems+10]+" "+unitsA[thisIsA[pieceIndex]*noOfItems+6]+"-"+unitsA[thisIsA[pieceIndex]*noOfItems+7]+" fire:"+unitsA[thisIsA[pieceIndex]*noOfItems+8]+"/"+unitsA[thisIsA[pieceIndex]*noOfItems+9]+" (pts:"+unitsA[thisIsA[pieceIndex]*noOfItems+2]+" ("+(typeNumbA[pieceIndex])+"/"+typeTotalA[pieceIndex]+")");
			...
			activeImage.setAttribute("title", titles2);
				...
				//console.log(activeIndex, unitsA[activeIndex*itemsPerUnit], unitsA[thisIsA[activeIndex]*itemsPerUnit]);
				  ...
				  printUnit(activeIndex, typeNumbA[activeIndex], typeTotalA[activeIndex], thisIsA[activeIndex],countIndividual[activeIndex]);
					...
					deadA[activeIndex]=0;//flag unit as alive - need to move this to before 'printing'
				}
...
function pointsStatus(s){
	//console.log("panic="+Apanic+"vp="+rebelVP)
  var sideThatDied = s;
...
function changeSupply(p, i, f){ //unit OUT OF SUPPLY
  ...
  if(flagChange==0){
    if(panicA[activeIndex]==2){panicA[activeIndex]=0;}
		...
		if(deadA[activeIndex]==0){
		if(panicA[activeIndex]!=1&&leaderA[activeIndex]==0){
			if(panicA[activeIndex]!=2){
				titles = activeImage.getAttribute("title");
...
function unDisrpAll(){
for(k=0;k<noOfImages;k++){
if(disruptedA[k]==1||disruptedA[k]==2){
	if(Math.random()<0.5){
activeIndex=k;
activeImage=imageArray[k];
	disruptedA[k]=0;
...
function unDisrpB(){
for(k=gerNumber;k<neutNumber;k++){
if(disruptedA[k]==1||disruptedA[k]==2){
	if(Math.random()<0.5){
activeIndex=k;
activeImage=imageArray[k];
...
for(s=0;s<gerNumber;s++){
	...
	document.getElementById('btnSA').innerText="supply A - all";
	}
	else if(supplyA==1){
for(s=0;s<gerNumber;s++){
		if(panicA[s]==0){
		...
		document.getElementById('btnSA').innerText="supply A - none";
	}
	else if(supplyA==2){
for(s=0;s<gerNumber;s++){
		if(Math.random()<0.5){
		...
		document.getElementById('btnSA').innerText="supply A - random";
	}
...
for(s=gerNumber;s<neutNumber;s++){
	...
	document.getElementById('btnSB').innerText="supply B - all";
	}
	else if(supplyB==1){
for(s=gerNumber;s<neutNumber;s++){
		if(panicA[s]==0){
		...
		document.getElementById('btnSB').innerText="supply B - none";
	}
	else if(supplyB==2){
for(s=gerNumber;s<neutNumber;s++){
		if(Math.random()<0.5){
		...
		document.getElementById('btnSB').innerText="supply B - random";
	}
	//**THIS ROUTINE FLIPS SUPPLY TO OPPOSITE
/*for(s=0;s<gerNumber;s++){
			changeSupply(imageArray[s], s, 1);	//becuse call it in loop need 'return' in changeSupply() to come back here
...
function RotSideB(){
for(s=gerNumber-1;s<neutNumber;s++){
  //alert(""+imageArray[k]+" "+k);
			changeRotR(imageArray[s], s, 1);	//becuse call it in loop need 'return' in changeSupply() to come back here
      //imageArray[k].style.borderStyle="solid";
			//movedA[k]=0;
...
function changeNeutralPic(n){//change, increase by 1
	var leaderIndex=n;
	leaderA[leaderIndex]++;
...
/*
		...
		printUnit(imgIndex, typeNumbA[imgIndex], typeTotalA[imgIndex], thisIsA[imgIndex],countIndividual[imgIndex]);
		hideA[imgIndex]=0;
		...
		hideGer=1;
		hideForces();
		document.getElementById("btnHA").innerText="Show B.";}
		else{
		hideGer=0;
		unHideForces();
		document.getElementById("btnHA").innerText="Hide B.";}
		...
		hideRus=1;
		hideForces();
		document.getElementById("btnHB").innerText="Show A.";
		}
		else{
		hideRus=0;
		unHideForces();
		document.getElementById("btnHB").innerText="Hide A.";
		}
		}
		...
		hideForces();
		unHideForces();
		document.getElementById("btnHH").innerText="Hide B./Show A.";
		}
		else{
		hideGer=1;
		hideRus=0;
		document.getElementById("btnHA").innerText="Show B.";
		document.getElementById("btnHA").innerText="Hide A.";
		hideForces();
		unHideForces();
		document.getElementById("btnHH").innerText="Show B./Hide A.";
		}
		}


			function hideHopper(){ //switch on/off Hopper
					    if(hopperFlag){hopperFlag=false;
					    keyBoard.style.zIndex = -10;
						document.getElementById('btnHp1').innerText="Show Hopper";}
					    else{hopperFlag=true;
					    keyBoard.style.zIndex = nextZ++;
						document.getElementById('btnHp1').innerText="Hide Hopper";}
					    }
