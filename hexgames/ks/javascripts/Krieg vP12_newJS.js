...
var boardBvary = mirrBrdNoB*10 + flipBrdNoB;
var boardRvary = mirrBrdNoR*10 + flipBrdNoR;
  	allText=allText+redArmyNo+","+blackArmyNo+","+redBno+","+ boardRvary +","+ blackBno +","+ boardBvary+",";
...
function reload(){
  ...
  var boardBvary = 1*loadA[loopTill-1-shiftBackwards];
  var boardRvary = 1*loadA[loopTill-3-shiftBackwards];
  mirrBrdNoB = Math.floor(boardBvary/10);
  mirrBrdNoR = Math.floor(boardRvary/10);
  flipBrdNoB = boardBvary-mirrBrdNoB*10;
  flipBrdNoR = boardRvary-mirrBrdNoR*10;
  ...
  if(mirrBrdNoR==1){mirrLetR = "R";}
  if(mirrBrdNoB==1){mirrLetB = "R";}
    document.getElementById('boardNameR').innerText="Red Board #"+redBno+flipLetR+mirrBrdNoR+"\n"+brdNameR;
    document.getElementById('boardNameB').innerText="Black Board #"+blackBno+flipLetB+mirrBrdNoB+"\n"+brdNameB;
...
if(turnLetter=="A"){
	...
	turnLetter="A";
	turnNumber++;
  if((turnNumber+startSeasonOffset)%12==0){
  yearNow=yearNow+1;}
}
var codeNumber = "0";
if(turnNumber<10){
...
//console.log(step3+" "+turnNumber+" "+startSeasonOffset+" "+seasonA[step3])
